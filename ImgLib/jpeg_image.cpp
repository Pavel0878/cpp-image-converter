    ...
    */
     ...
     */
        ...
        for(int x = 0; x < cinfo.image_width; ++x) {
            image_buffer[x * 3 + 0] = JSAMPLE(image.GetLine(cinfo.next_scanline)[x].r);
            image_buffer[x * 3 + 1] = JSAMPLE(image.GetLine(cinfo.next_scanline)[x].g);
            image_buffer[x * 3 + 2] = JSAMPLE(image.GetLine(cinfo.next_scanline)[x].b);
        }
